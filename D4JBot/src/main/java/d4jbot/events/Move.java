...
public class Move implements Command{
	...
	public void onMessageReceivedEvent(MessageReceivedEvent e) {
		if(e.getMessage().getContent().startsWith(BotSettings.BOT_PREFIX.getPropertyValue() + "move")) {
			...
			if(args.length == 3) {
				try {
					...
					} else ms.sendMessage(BoundChannel.MUSIC.getBoundChannel(), songToMove + " or " + position + " is not a valid song ID.");

				} catch(NumberFormatException e2) {
					ms.sendMessage(BoundChannel.MUSIC.getBoundChannel(), "One of the provided song IDs is not a number");
				}
			} else ms.sendMessage(BoundChannel.MUSIC.getBoundChannel(), "Invalid usage of $move\nSyntax: $move <songID> <songID>");
		}
	}
}
