...
public class ToggleCommand {
	public static Mono<Void> execute(MessageCreateEvent event, Guild guild, Member member, MessageChannel channel, List<String> params) {
		...
		if (commands.isEmpty()) {
			String message = String.format(getLocalisation(LocalisationKey.ERROR_NOT_REAL_COMMAND, guild).getValue(), params.get(0));
		...
		if (ms.isEnabled()) {
			message = getLocalisation(LocalisationKey.COMMAND_TOGGLE_ON, guild);
		} else {
			message = getLocalisation(LocalisationKey.COMMAND_TOGGLE_OFF, guild);
		}
